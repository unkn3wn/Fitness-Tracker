...
const{getAllActivities, createActivites,updateActivity, getActivityById} = require("../db/adapters/activites");
...
actRouter.patch("/:activityId", authRequired, async(req,res,next)=>{
    ...
    try{
        ...
        if(oA.id === req.user.id){
            ...
            next({
                name:"NOT AUTH",
                message:"not allowed to update activity if not yours or please login/register to an existing user"
            })
        }
    }catch({name, message}){
        next({name, message});
